...
const NEXTCLOUD_URL = process.env.NEXTCLOUD_URL || 'http://localhost:8084';
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // 上传到 Nextcloud
    const uploadUrl = `${NEXTCLOUD_URL}/remote.php/webdav/${encodeURIComponent(fullPath).replace(/%2F/g, '/')}`;
    ...
    let publicUrl = `${NEXTCLOUD_PUBLIC_URL}/remote.php/webdav/${encodeURIComponent(fullPath).replace(/%2F/g, '/')}`;
